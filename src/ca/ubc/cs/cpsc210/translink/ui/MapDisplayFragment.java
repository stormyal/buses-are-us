...
public class MapDisplayFragment extends Fragment implements MapEventsReceiver, IMyLocationConsumer {
    ...
    private TopRightTextOverlay busRouteLegendOverlay;
    ...
    /**
     * Return a new stop cluster object used to group stops that are close by to reduce screen clutter
     */
    ...
    private void createBusRouteLegendOverlay() {
        ...
        busRouteLegendOverlay = new TopRightTextOverlay(rp, dpiFactor());
    }
    ...
    private void plotRoutes() {
        ...
        StopSelectionListener listener = (StopSelectionListener) getActivity();
        Stop s = StopManager.getInstance().getSelected();
        busRouteLegendOverlay.clear();
        if (s != null) {
            listener.onStopSelected(s);
            for (Route r : s.getRoutes()) {
                ...
                for (RoutePattern rp : r.getPatterns()) {
                    ...
                    for (int i = 0; i < temp.size()-1; i++) {
                        if (Geometry.rectangleIntersectsLine(northWest, southEast, temp.get(i), temp.get(i+1))) {
                            geoPoints = new ArrayList<>();
                            geoPoints.add(new GeoPoint(temp.get(i).getLatitude(), temp.get(i).getLongitude()));
                            geoPoints.add(new GeoPoint(temp.get(i+1).getLatitude(), temp.get(i+1).getLongitude()));
                            Polyline routeLine = new Polyline(getActivity());
                            routeLine.setPoints(geoPoints);
                            routeLine.setVisible(true);
                            routeLine.setWidth(this.getLineWidth(mapView.getZoomLevel()));
                            routeLine.setColor(busRouteLegendOverlay.getColor(r.getNumber()));
                            busRouteOverlays.add(routeLine);
                        }
                    }
                    temp.clear();
                }
...
//                + southeast.getLatitude() + " " + southeast.getLongitude());
        ...
        for (Stop s: StopManager.getInstance()){
            if (Geometry.rectangleContainsPoint(northWest, southEast, s.getLocn())) {
                if (nearestStopMarker != null) {
                    if (s.equals(nearestStopMarker.getRelatedObject())) {
                ...
                stopMarker.setPosition(new GeoPoint(s.getLocn().getLatitude(), s.getLocn().getLongitude()));
                stopMarker.setInfoWindow(stopInfoWindow);
                stopMarker.setTitle(s.getNumber() + " " + s.getName());
                stopMarker.setSubDescription(s.getRoutes().toString());
    ...
    private void updateMarkerOfNearest(Stop nearest) {
        ...
        if (nearestStopMarker != null) {
            ...
            nearestStopMarker.setTitle(nearest.getNumber() + " " + nearest.getName());
    ...
    private void handleLocationChange(Location location) {
        LatLon currentLocation = new LatLon(location.getLatitude(), location.getLongitude());
        Stop nearest = StopManager.getInstance().findNearestTo(currentLocation);
//        centerAt(new GeoPoint(location.getLatitude(), location.getLongitude()));
        updateMarkerOfNearest(nearest);
        locationListener.onLocationChanged(nearest, currentLocation);
